            ...
            for (int i = 0; i < 3; ++i)
            {
                if(i != PlayerId)
                {
                    ...
                    if (confidence0 > 0 && myHair != Noun.Blonde)
                        PlayerInteraction.Get().QueueDialogue(sprite, "I think BLONDE did it (confidence " + confidence0 + ")");
                    else if (confidence1 > 0 && myHair != Noun.Brown)
                        PlayerInteraction.Get().QueueDialogue(sprite, "I think BROWN did it (confidence " + confidence1 + ")");
                    else if (confidence2 > 0 && myHair != Noun.Red)
                        PlayerInteraction.Get().QueueDialogue(sprite, "I think RED did it (confidence " + confidence2 + ")");
                    else
                    {
                        ...
                        if (myHair == Noun.Blonde)
                        {
                            if (innocenceBrown > 0f || innocenceRed > 0f)
                            {
                                ...
                                PlayerInteraction.Get().QueueDialogue(sprite, "Well I didn't do it, and " + innocentName + " didn't do it, so " + guiltyName + " did.");
                            }
                            else
                            {
                                PlayerInteraction.Get().QueueDialogue(sprite, "I have no idea.");
                            }
                        }
